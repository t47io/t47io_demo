...
$.ajax({
    ...
    success: function (data) {
        ...
        $("#recent_flash").html('<span class="label label-primary">' + data.flash_slide.date + '</span>&nbsp;&nbsp;<a href="' + data.flash_slide.url + '" target="_blank" class="link"><span class="glyphicon glyphicon-new-window"></span></a>');
        ...
        $("#recent_khala").html('<i>' + data.khala.name + '</i>&nbsp;&nbsp;<span class="label label-primary">' + data.khala.date + '</span>&nbsp;&nbsp;<a href="' + data.khala.url + '" target="_blank" class="link"><span class="glyphicon glyphicon-new-window"></span></a>');
        $("#recent_rotation").html('<i>' + data.rotation.name + '</i>&nbsp;&nbsp;<span class="label label-primary">' + data.rotation.date + '</span>&nbsp;&nbsp;<a href="/site_data/rot_ppt/' + data.rotation.url + '" target="_blank" class="link"><span class="glyphicon glyphicon-new-window"></span></a>');
        $("#recent_archive").html('<i>' + data.archive.name + '</i>&nbsp;&nbsp;<span class="label label-primary">' + data.archive.date + '</span>&nbsp;&nbsp;<a href="/site_data/spe_ppt/' + data.archive.url + '" target="_blank" class="link"><span class="glyphicon glyphicon-new-window"></span></a>');
...
$.ajax({
    ...
    success: function (data) {
        ...
        for (var i = 0; i < data.users.length / 2; i++) {
            ...
            html += '<tr><td><a target="_blank" class="mail_link" href="mailto:' + u1.email + '"><span class="glyphicon glyphicon-envelope"></span></a>&nbsp;&nbsp;' + presence1.replace('__TS__', u1.last_seen) + '</span>&nbsp;&nbsp;<div class="sprite" style="width:24px; height:24px;"><div class="avatar_' + u1.id + '"></div></div>&nbsp;&nbsp;<i>' + u1.name + '</i></td>';
            if (typeof u2 === "undefined") {
                html += '<td></td></tr>';
            } else {
                if (u2.presence) { presence2 = presence2.replace('presence_no', 'presence_yes').replace('unchecked', 'check'); }
                html += '<td><a target="_blank" class="mail_link" href="mailto:' + u2.email + '"><span class="glyphicon glyphicon-envelope"></span></a>&nbsp;&nbsp;' + presence2.replace('__TS__', u2.last_seen) + '</span>&nbsp;&nbsp;<div class="sprite" style="width:24px; height:24px;"><div class="avatar_' + u2.id + '"></div></div>&nbsp;&nbsp;<i>' + u2.name + '</i></td></tr>';
            }
        }
