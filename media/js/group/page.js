if ((app.key == "meeting" && (app.page == "journal_club" || app.page == "khala_youtube" || app.page == "rotation")) || (app.key == "res" && (app.page == "archive" || app.page == "defense"))) {
    ...
    $(".ppt_link").on("click", function(e) {
        e.preventDefault();
        $("#sys-head").html("Presentation Link");
        $("#sys-msg").html("This will redirect you to download Presentation File (not included for this demo).");
        $("#sys-modal").modal("show");
        setTimeout(function() { $("#sys-modal").modal("hide"); }, 1000);
    });
    $(".url_link").on("click", function(e) {
        e.preventDefault();
        $("#sys-head").html("External Link");
        $("#sys-msg").html("This will redirect you to the External link (not included for this demo).");
        $("#sys-modal").modal("show");
        setTimeout(function() { $("#sys-modal").modal("hide"); }, 1000);
    });
    $(".dat_link").on("click", function(e) {
        e.preventDefault();
        $("#sys-head").html("Data File");
        $("#sys-msg").html("This will redirect you to download Data File (not included for this demo).");
        $("#sys-modal").modal("show");
        setTimeout(function() { $("#sys-modal").modal("hide"); }, 1000);
    });
    $("#yt_link").on("click", function(e) {
        e.preventDefault();
        $("#sys-head").html("YouTube Channel");
        $("#sys-msg").html("This will redirect you to the YouTube Channel link (not included for this demo).");
        $("#sys-modal").modal("show");
        setTimeout(function() { $("#sys-modal").modal("hide"); }, 1000);
    });
    $(".yt_link").on("click", function(e) {
    ...
    if (app.page == "schedule") {
        ...
        $(".flash_link").on("click", function(e) {
            e.preventDefault();
            $("#sys-head").html("Flash Slide");
            $("#sys-msg").html("This will redirect you to the Google Docs link (not included for this demo).");
            $("#sys-modal").modal("show");
            setTimeout(function() { $("#sys-modal").modal("hide"); }, 1000);
        });
    }
    ...
    if (app.page == "archive/upload") {
        ...
        $("#form_change_submit").on("click", function(e) {
            e.preventDefault();
            $("#sys-head").html("Update Contact");
            $("#sys-msg").html("This will update member contact (not included for this demo).");
            $("#sys-modal").modal("show");
            setTimeout(function() { $("#sys-modal").modal("hide"); }, 1000);
        });
        ...
        $(".cap_link").on("click", function(e) {
            e.preventDefault();
            $("#sys-head").html("CAP Profile Link");
            $("#sys-msg").html("This will redirect you to CAP profile (not included for this demo).");
            $("#sys-modal").modal("show");
            setTimeout(function() { $("#sys-modal").modal("hide"); }, 1000);
        });

    } else if (app.page == "gdocs") {
        $(".doc_link").on("click", function(e) {
            e.preventDefault();
            $("#sys-head").html("Google Drive File");
            $("#sys-msg").html("This will redirect you to the Google Drive link (not included for this demo).");
            $("#sys-modal").modal("show");
            setTimeout(function() { $("#sys-modal").modal("hide"); }, 1000);
        });
    }
