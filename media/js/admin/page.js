if (typeof app.fnFormatInput !== "function") {
    function replace_path(string) {
        return string.replace('demo/', '/site_data/').replace('Website_Server/Daslab/data/', '/site_data/');
    }
...
if (app.page == "backup" || app.page == "bot" || app.page == "export") {
    ...
    $("div.object-tools > a").on("click", function(e) {
        e.preventDefault();
        $("#sys-head").html("Add User");
        $("#sys-msg").html("This will add a new user login (not included for this demo).");
        $("#sys-modal").modal("show");
        setTimeout(function() { $("#sys-modal").modal("hide"); }, 1000);
    });

    $("#id_password").next().children().attr("id", "change_pwd").css("text-decoration", "line-through");
    ...
    $("div.object-tools > a").on("click", function(e) {
        e.preventDefault();
        $("#sys-head").html("Slack Message");
        $("#sys-msg").html("This will add a slack message (not included for this demo).");
        $("#sys-modal").modal("show");
        setTimeout(function() { $("#sys-modal").modal("hide"); }, 1000);
    });
...
if (["apache", "aws", "ga", "git", "dir", "backup", "export", "man", "ref"].indexOf(app.page) == -1 && app.key != 'home') {
    ...
    $("#content a:not(#btn-slack-add):not(#btn-auth-add):not(#change_pwd):not(.form-file-view)").on("click", function(event) {
